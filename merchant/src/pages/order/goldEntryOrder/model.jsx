...
const Model = {
  ...
  effects: {
    *fetch({ payload }, { select,call, put }) {
      let response = {};
      const his = yield select(state => state.goldEntryOrder.data.history);
      if(window.location.href.indexOf('?history') > -1 && Object.keys(his).length != 0){
        ...
        response = yield call(goldEntryOrderSearchAll, payload);
      }else{
        response = yield call(goldEntryOrder, payload);
      }
