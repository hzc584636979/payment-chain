      ...
      <ContLayout>
        <div className={styles.wrap}>
          <Descriptions column={1}>
            ...
            <Descriptions.Item label="订单金额(CNY)">{ goldEntryOrderDetail.pay_amount_cny }</Descriptions.Item>
            <Descriptions.Item label="单价(CNY)">{ goldEntryOrderDetail.cny_price }</Descriptions.Item>
            <Descriptions.Item label="手续费(USDT)">{ goldEntryOrderDetail.gas }</Descriptions.Item>
            <Descriptions.Item label="创建时间">{ moment(goldEntryOrderDetail.created_at).local().format('YYYY-MM-DD HH:mm:ss') }</Descriptions.Item>
          </Descriptions>
        </div>
      </ContLayout>
    );
