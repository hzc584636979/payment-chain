      ...
      <ContLayout>
        <div className={styles.wrap}>
          <Descriptions column={1}>
            <Descriptions.Item label="订单状态">{ buyStatusType[goldYieldDissentOrderDetail.state] }</Descriptions.Item>
            <Descriptions.Item label="异议时间">{ moment(goldYieldDissentOrderDetail.issue_create_time).local().format('YYYY-MM-DD HH:mm:ss') }</Descriptions.Item>
            <Descriptions.Item label="问题类型">{ goldYieldDissentOrderDetail.issue_type }</Descriptions.Item>
            <Descriptions.Item label="问题描述">{ goldYieldDissentOrderDetail.issue_desc }</Descriptions.Item>
            <Descriptions.Item label="平台订单号">{ goldYieldDissentOrderDetail.order_id }</Descriptions.Item>
            <Descriptions.Item label="商户订单号">{ goldYieldDissentOrderDetail.out_order_id }</Descriptions.Item>
            <Descriptions.Item label="承兑商姓名">{ goldYieldDissentOrderDetail.a_user_name }</Descriptions.Item>
            <Descriptions.Item label="币种">{ coinType[goldYieldDissentOrderDetail.token_id] }</Descriptions.Item>
            <Descriptions.Item label="订单金额（USDT）">{ goldYieldDissentOrderDetail.pay_amount }</Descriptions.Item>
            <Descriptions.Item label="订单金额（CNY）">{ goldYieldDissentOrderDetail.pay_amount_cny }</Descriptions.Item>
            <Descriptions.Item label="订单创建时间">{ moment(goldYieldDissentOrderDetail.created_at).local().format('YYYY-MM-DD HH:mm:ss') }</Descriptions.Item>
            ...
            <div className={styles.layerWrap} style={{width: '70%', margin: '0 auto'}}>
              <Descriptions column={1}>
                <Descriptions.Item label={<span className={styles.itemLabel}>联系方式</span>}>
                  <Input placeholder="输入联系方式" maxLength={11} style={{width: '100%'}} onChange={this.handleContact} />
                </Descriptions.Item>
