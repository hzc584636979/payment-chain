...
@connect(({ user, yieldErc20, loading }) => ({
  ...
  yieldErc20,
  loading: loading.effects['yieldErc20/fetch'],
}))
@Form.create()
class YieldErc20 extends Component {
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    const { dispatch } = this.props;
    const { history } = this.props.yieldErc20.data;
  ...
  yield = record => {
    const { dispatch } = this.props;
    const { MM } = this.state;
    const { pagination } = this.props.yieldErc20.data;
  ...
  render() {
    ...
    const { history, list, pagination } = this.props.yieldErc20.data;
