import { yieldErc20List, yieldErc20ListSearchAll, yieldErc20 } from '@/services/api';
...
const Model = {
  ...
  effects: {
    *fetch({ payload }, { select,call, put }) {
      let response = {};
      const his = yield select(state => state.yieldErc20.data.history);
      if(window.location.href.indexOf('?history') > -1 && Object.keys(his).length != 0){
        ...
        response = yield call(yieldErc20ListSearchAll, payload);
      }else{
        response = yield call(yieldErc20List, payload);
      }
    ...
    *search({ payload }, { call, put }) {
      const response = yield call(yieldErc20ListSearchAll, payload);
      let { rows, count } = response.data || {};
      let page = payload && payload.page;
      let pageSize = payload && payload.pageSize;
      const responseResult = { list: rows, pagination: { total: count, current: page+1, pageSize }, history: { ...payload } };
    ...
    *yield({ payload }, { call, put }) {
      const response = yield call(yieldErc20, { ...payload, token_id: 1});
      return response;
    },
  },
  reducers: {
