...
class Home extends Component {
  ...
  handleWithdrawApply = maxCoin => {
    ...
    const {
      withdrawApplyType,
      withdrawApplyValue,
      withdrawApplyAddress,
      withdrawApplyCaptcha,
      walletType,
    } = this.state;
  ...
  render() {
    ...
    const allLockBalance = currentUser.id
      ? new BigNumber(wei2USDT(currentUser.erc20.lock_balance))
          ...
          .toNumber()
      : 0;
    const confirmations = currentUser.id
      ? walletType == '1'
        ? currentUser.erc20.confirmations
        : currentUser.omni.confirmations
      : null;
    ...
    return (
      <Spin spinning={!!getUserInfoLoading}>
        <GridContent>
                ...
                <Col xl={5} md={24} sm={24} xs={24}>
                  <div className={`${styles.layoutLeft} ${styles.userInfo}`}>
                    <div className={styles.avatar}>
                      <img src={currentUser.logo_path || DEFAULTAVATAR} />
                    </div>
                    <div className={styles.user}>
                      {currentUser.user_name || <Link to="/account">设置名称</Link>}
                      <br />
                    ...
                    <div className={styles.message3}>
                      ...
                      >
                        <Col style={{ fontSize: 16 }}>
                          ...
                          ></span>
                          <span style={{ color: '#CF0000' }}>[重要通知]</span>
                          {currentUser.real_name_passed
                            ? ` 您提交的支付链商家认证，已通过审核`
                            : ` 您提交的支付链商家认证，后台正在审核中`}
                        </Col>
                          ...
                          <div className={styles.itemBox}>
                            ...
                            <div className={styles.item}>
                              ...
                              <span
                                ...
                                onClick={() => this.eyeVisible('tokenBalance1')}
                              >
                                {tokenBalance1 ? (
                            ...
                            <div className={styles.item}>
                              ...
                              <span
                                style={{ cursor: 'pointer', marginLeft: 5 }}
                                onClick={() => this.eyeVisible('tokenBalance2')}
                              >
                                {tokenBalance2 ? (
                                  <Icon type="eye-invisible" />
                                ) : (
                      ...
                      >
                        ...
                        <Col xl={8} md={8} sm={12} xs={12}>
                          <div className={styles.item}>
                            ...
                            <div className={styles.but}>
                              <Link to="/dissentOrder/goldYieldOrder">
                                <Button type="primary">去处理</Button>
                              </Link>
                            </div>
                          </div>
                        ...
                        <Col xl={8} md={8} sm={12} xs={12}>
                          <div className={styles.item}>
                            ...
                            <div className={styles.but}>
                              <Link to="/dissentOrder/goldEntryOrder">
                                <Button type="primary">去处理</Button>
                              </Link>
                            </div>
                          </div>
          ...
          {payVisible && (
            <Layer title={`充值`} hiddenVisible={this.handlePayVisible}>
              <div className={styles.payLayout}>
                ...
                <div className={styles.desc}>
                  温馨提示：
                  <br />
                  充值USDT需要{confirmations}个区块确认，请耐心等待。此地址只接受
                  {walletType == 1 ? 'erc20' : 'omni'}
                  协议的USDT，请勿往地址元值其他协议的USDT发送其他币种到此地址将无法找回，平台也不承担带来的损失。
                </div>
              </div>
            </Layer>
          )}
          {withdrawApplyVisible && (
            <Layer title={`提币`} hiddenVisible={this.handleWithdrawApplyVisible}>
              <div className={styles.withdrawApplyLayout}>
                <div style={{ textAlign: 'center' }}>
                  ...
                  <Row>
                    ...
                    <Col xl={14} md={13} sm={24} xs={24} style={{ marginRight: 10 }}>
                      <Input
                        placeholder="输入提币地址"
                        onChange={this.handleWithdrawApplyAddress}
                        value={withdrawApplyAddress}
                      />
                    </Col>
                  </Row>
                  <Row>
                    ...
                    <Col xl={14} md={13} sm={24} xs={24} style={{ marginRight: 10 }}>
                      <Input
                        placeholder="输入提币数量"
                        onChange={this.handleWithdrawApplyNumber}
                        value={withdrawApplyValue}
                      />
                    </Col>
                  ...
                  <Row style={{ marginBottom: 40 }}>
                    <Col
                      ...
                      sm={0}
                      xs={0}
                      style={{ lineHeight: '32px', fontSize: 16, color: '#666666' }}
                    ></Col>
                    <Col
                      xl={14}
                      md={13}
                  ...
                  <Row>
                    ...
                    <Col xl={14} md={13} sm={24} xs={24} style={{ marginRight: 10 }}>
                      <Input
                        placeholder="输入短信验证码"
                        maxLength={6}
                        onChange={this.handleWithdrawApplyCaptcha}
                        value={withdrawApplyCaptcha}
                      />
                    </Col>
                    <Col xl={5} md={5} sm={24} xs={24}>
                      <Button
                ...
                <div className={styles.desc}>
                  温馨提示：
                  <br />
                  1.提币所需手续费，手续费为{currentUser.gas} USDT；
                  <br />
                  2.请务必确保提币地址的正确性，若由于地址填写错误导致资金丢失，不属于平台责任。
                </div>
                <div style={{ textAlign: 'center' }}>
                  <Button
                    loading={withdrawApplyStatus}
                    type="primary"
                    style={{ width: 120 }}
                    onClick={() =>
                      this.handleWithdrawApply(
                        new BigNumber(allBalance)
                          .minus(new BigNumber(allLockBalance))
                          .minus(new BigNumber(currentUser.gas))
                          .toNumber(),
          ...
          {!getRealNamePassed() && (
            <div className={styles.passedLayer}>
              用户必须通过实名认证后方可使用平台的功能{' '}
              <span onClick={this.checkUserAuth} style={{ color: '#EA0000', cursor: 'pointer' }}>
                去认证
              </span>
            </div>
          )}
