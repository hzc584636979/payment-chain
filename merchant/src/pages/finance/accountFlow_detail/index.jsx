        ...
        <div className={styles.wrap}>
          <Descriptions column={1}>
            <Descriptions.Item label="订单状态">
              {buyStatusType[coinDissentOrderDetail.state]}
            </Descriptions.Item>
            <Descriptions.Item label="平台订单号">
              {coinDissentOrderDetail.order_id}
            </Descriptions.Item>
            <Descriptions.Item label="商户订单号">
              {coinDissentOrderDetail.out_order_id}
            </Descriptions.Item>
            <Descriptions.Item label="付款方式">
              <img src={payIcon[coinDissentOrderDetail.pay_type]} />
            </Descriptions.Item>
            <Descriptions.Item label="付款姓名">
              {coinDissentOrderDetail.payee_name}
            </Descriptions.Item>
            <Descriptions.Item label="银行卡号">{buyOrderDetail.payee_account}</Descriptions.Item>
            <Descriptions.Item label="银行名称">
              {coinDissentOrderDetail.account_bank_name}
            </Descriptions.Item>
            ...
            <Descriptions.Item label="代币数量">
              {coinDissentOrderDetail.pay_amount}
            </Descriptions.Item>
            <Descriptions.Item label="创建时间">
              {moment(coinDissentOrderDetail.created_at)
                .local()
                .format('YYYY-MM-DD HH:mm:ss')}
            </Descriptions.Item>
            <Descriptions.Item label="转账时间">
              {coinDissentOrderDetail.transfer_time
                ? moment(coinDissentOrderDetail.transfer_time)
                    .local()
                    .format('YYYY-MM-DD HH:mm:ss')
                : EXHIBITION2}
            </Descriptions.Item>
            <Descriptions.Item label="承兑商确认时间">
              {coinDissentOrderDetail.confirm_time
                ? moment(coinDissentOrderDetail.confirm_time)
                    .local()
                    .format('YYYY-MM-DD HH:mm:ss')
                : EXHIBITION2}
            </Descriptions.Item>
