...
const CreateModifyForm = Form.create()(props => {
  ...
  return (
    ...
    >
      <Form>
        <div style={{paddingBottom: '28px', textAlign: 'center'}}>承兑商 <span style={{color: '#308AFF'}}>{ params && params.user_name }</span> 可管理人数 <span style={{color: '#EB9211'}}>{ params && params.manageable_peoples }</span></div>
...
@connect(({ superAccept, loading }) => ({
  superAccept,
  loading: loading.effects['superAccept/fetch'],
}))
@Form.create()
class SuperAccept extends Component {
  state = {
    selectedRowKeys: [],
  };
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    const { dispatch } = this.props;
    const { history } = this.props.superAccept.data;
  ...
  render() {
    ...
    const { history, list, pagination } = this.props.superAccept.data;
      ...
      },
      {
        ...
        render: (val, record) => {
          return(
            <span>
              <Button>
                <Link to={`/super/accept_add/${record.id}?parent_name=${record.user_name}`}>成员管理</Link>
              </Button>
