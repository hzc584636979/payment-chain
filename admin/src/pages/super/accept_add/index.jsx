...
class SuperAcceptMember extends Component {
  ...
  componentDidMount() {
    this.parent_id = pathToRegexp('/super/accept_add/:id').exec(window.location.pathname)[1];
    const { dispatch } = this.props;
    dispatch({
      type: 'superAcceptMember/fetch',
      payload:{
        pageSize: 10,
        page: 0,
        search_value: null,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    const { dispatch } = this.props;
    const { history } = this.props.superAcceptMember.data;
  ...
  add = () => {
    const { dispatch } = this.props;
    const { pagination } = this.props.superAcceptMember.data;
...
export default SuperAcceptMember;
