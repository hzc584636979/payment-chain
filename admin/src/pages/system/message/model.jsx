import { systemMessage, systemMessageSearchAll, systemMessageDelete } from '@/services/api';
...
const Model = {
  ...
  effects: {
    *fetch({ payload }, { select,call, put }) {
      let response = {};
      const his = yield select(state => state.systemMessage.data.history);
      if(window.location.href.indexOf('?history') > -1 && Object.keys(his).length != 0){
        ...
        response = yield call(systemMessageSearchAll, payload);
      }else{
        response = yield call(systemMessage, payload);
      }
    ...
    *search({ payload }, { call, put }) {
      const response = yield call(systemMessageSearchAll, payload);
      let { rows, count } = response.data || {};
      let page = payload && payload.page;
      let pageSize = payload && payload.pageSize;
      const responseResult = { list: rows, pagination: { total: count, current: page+1, pageSize }, history: { ...payload } };
