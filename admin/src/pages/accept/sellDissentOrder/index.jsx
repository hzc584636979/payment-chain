...
@connect(({ acceptSellDissentOrder, loading }) => ({
  acceptSellDissentOrder,
  loading: loading.effects['acceptSellDissentOrder/fetch'],
  searchLoading: loading.effects['acceptSellDissentOrder/search'],
}))
@Form.create()
class AccpetSellDissentOrder extends Component {
  state = {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'acceptSellDissentOrder/fetch',
      payload:{
        pageSize: 10,
        page: 0,
        order_id: null,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    const { dispatch } = this.props;
    const { history } = this.props.acceptSellDissentOrder.data;
    ...
    dispatch({
      type: 'acceptSellDissentOrder/search',
      payload: params,
    });
  };
  ...
  render() {
    ...
    const { history, list, pagination } = this.props.acceptSellDissentOrder.data;
    const columns = [
      {
        title: '订单创建时间',
      ...
      },
      {
        ...
        render: (val, record) => {
          return(
            <span>
              <Button>
                <Link to={`/accept/sellDissentOrder_detail/${record.order_id}`}>查看</Link>
              </Button>
            </span>
          );
        },
...
export default AccpetSellDissentOrder;
