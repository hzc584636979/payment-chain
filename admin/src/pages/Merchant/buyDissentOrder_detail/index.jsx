...
class MerchantBuyDissentOrderDetail extends Component {
  ...
  render() {
    ...
    if(merchantBuyDissentOrderDetail.complainant == 1) {
    	appeal = merchantBuyDissentOrderDetail.m_user_name,
    	appeal_telephone_number = merchantBuyDissentOrderDetail.m_telephone_number,
    	beAppeal = merchantBuyDissentOrderDetail.a_user_name,
    	beAppeal_telephone_number = merchantBuyDissentOrderDetail.a_telephone_number;
    }else {
    	appeal = merchantBuyDissentOrderDetail.a_user_name,
    	appeal_telephone_number = merchantBuyDissentOrderDetail.a_telephone_number,
    	beAppeal = merchantBuyDissentOrderDetail.m_user_name,
    	beAppeal_telephone_number = merchantBuyDissentOrderDetail.m_telephone_number;
    }
      ...
      <ContLayout>
        <div className={styles.wrap}>
          <Descriptions column={1}>
            <Descriptions.Item label="异议订单时间">{ moment(merchantBuyDissentOrderDetail.updated_at).local().format('YYYY-MM-DD HH:mm:ss') }</Descriptions.Item>
            ...
            <Descriptions.Item label="申诉者描述">{ merchantBuyDissentOrderDetail.issue_desc }</Descriptions.Item>
            <Descriptions.Item label="申诉者手机号">{ appeal_telephone_number }</Descriptions.Item>

            {
              merchantBuyDissentOrderDetail.issue_state == 3 &&
              <Fragment>
                <Descriptions.Item label="客服介入者联系方式">{ merchantBuyDissentOrderDetail.contact }</Descriptions.Item>
                <Descriptions.Item label="客服介入者申诉描述">{ merchantBuyDissentOrderDetail.content }</Descriptions.Item>
              </Fragment>
            }
...

            {
              ...
              <Fragment>	
            	<Descriptions.Item label="管理员处理时间">{ moment(merchantBuyDissentOrderDetail.manager_deal_time).local().format('YYYY-MM-DD HH:mm:ss') }</Descriptions.Item>
            	<Descriptions.Item label="管理员姓名">{ merchantBuyDissentOrderDetail.manager_name }</Descriptions.Item>
            	<Descriptions.Item label="管理员手机号">{ merchantBuyDissentOrderDetail.manager_telephone_number }</Descriptions.Item>
              	<Descriptions.Item label="处理结果">{ merchantBuyDissentOrderDetail.issue_result }</Descriptions.Item>
              	{
              		merchantBuyDissentOrderDetail.forfiet && 
              		<Descriptions.Item label="惩罚金额">{ `${merchantBuyDissentOrderDetail.forfiet}USDT` }</Descriptions.Item>
              	}
              </Fragment>
            }
