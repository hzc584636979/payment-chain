...
@connect(({ merchantDissentOrder, loading }) => ({
  merchantDissentOrder,
  loading: loading.effects['merchantDissentOrder/fetch'],
}))
@Form.create()
class MerchantDissentOrder extends Component {
  state = {
  ...
  componentDidMount() {
    ...
    dispatch({
      type: 'merchantDissentOrder/fetch',
      payload:{
        pageSize:10,
        page:0,
        order_id: null,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    const { dispatch } = this.props;
    const { history } = this.props.merchantDissentOrder.data;
    ...
    dispatch({
      type: 'merchantDissentOrder/search',
      payload: params,
    });
  };
  ...
  render() {
    ...
    const { history, list, pagination } = this.props.merchantDissentOrder.data;
    const columns = [
      {
        title: '异议时间',
...
export default MerchantDissentOrder;
